...
import { registerEnum } from "./lib.register-enum";
...
registerEnum(LikeStatus, "LikeStatus", `Describes whether a like has been created or deleted in the database.`);
...
export class CreateOrUpdateLikes {
  ...
  async createOrUpdateLikes(
    ...
    if (!retrievedLikes) {
      ...
      let likeReturn: LikeReturnType = {
        postId: input.postId,
        status:
          newlyCreatedLike.raw[0].id && typeof newlyCreatedLike.raw[0].id === "string"
            ? LikeStatus.Created
  ...
  @Subscription(() => LikeReturnType, {
    ...
    filter: ({ payload, args }: ResolverFilterData<LikeReturnType, UpdateLikesInput>) => {
